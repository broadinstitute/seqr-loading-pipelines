...
ht = ht.annotate_globals(datasets=ht.datasets.remove('1kg'))
ht = ht.checkpoint('gs://seqr-scratch-temp/37.ht')
...
ht = ht.annotate_globals(datasets={k: v for k, v in ht.datasets.collect()[0].items() if k!='1kg'})
ht = ht.annotate_globals(version='2.0.5')
ht = ht.checkpoint('gs://seqr-scratch-temp/38.ht')

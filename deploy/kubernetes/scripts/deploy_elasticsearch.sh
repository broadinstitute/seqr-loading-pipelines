...
if [ "$DELETE_BEFORE_DEPLOY" ]; then
    kubectl delete -f kubernetes/settings/elasticsearch/elasticsearch.${DEPLOY_TO_PREFIX}.yaml
...
kubectl apply -f kubernetes/settings/elasticsearch/elasticsearch.${DEPLOY_TO_PREFIX}.yaml --record

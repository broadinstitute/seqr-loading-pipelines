...
if [ "$DELETE_BEFORE_DEPLOY" ]; then
    kubectl delete -f kubernetes/configs/kibana/kibana.${DEPLOY_TO_PREFIX}.yaml
...
docker build $CACHE_ARG --build-arg KIBANA_SERVICE_PORT=$KIBANA_SERVICE_PORT -t ${DOCKER_IMAGE_PREFIX}/kibana docker/kibana/
...
kubectl apply -f kubernetes/configs/kibana/kibana.${DEPLOY_TO_PREFIX}.yaml --record

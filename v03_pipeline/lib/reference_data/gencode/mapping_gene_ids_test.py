...
class LoadGencodeTestCase(unittest.TestCase):
    ...
    @mock.patch(
        'v03_pipeline.lib.reference_data.gencode.mapping_gene_ids.download_file',
    )
    def test_load_gencode_local(
        self,
        mock_download_file,
        mock_file_writer,
        mock_gopen,
        ...
        mock_file_writer.assert_called_with(
            'test/path/gencode.v24.annotation.gtf.pickle',
        )
        ...
        mock_logger.info.assert_has_calls(
            [
                ...
                mock.call(
                    'Saving to pickle test/path/gencode.v24.annotation.gtf.pickle',
                ),
                mock.call('Got 3 gene id mapping records'),
            ],
        )
    ...
    @mock.patch('v03_pipeline.lib.reference_data.gencode.mapping_gene_ids.pickle')
    @mock.patch(
        'v03_pipeline.lib.reference_data.gencode.mapping_gene_ids.stream_gs_file',
    )
    @mock.patch('v03_pipeline.lib.reference_data.gencode.mapping_gene_ids.file_writer')
    def test_load_gencode_using_gs(
        self,
        mock_file_writer,
        mock_stream_gs_file,
        mock_pickle,
        ...
        self.assertEqual(gene_id_mapping, GENE_ID_MAPPING)
        mock_path_exists.assert_called_with(
            'gs://test-bucket/test/path/gencode.v25.annotation.gtf.pickle',
        )
        mock_logger.info.assert_has_calls(
            [
                mock.call(
                    'Use the existing pickle file gs://test-bucket/test/path/gencode.v25.annotation.gtf.pickle.\n'
                    'If you want to reload the data, please delete it and re-run the data loading.',
                ),
                mock.call('Got 3 gene id mapping records'),
            ],
        )
        mock_stream_gs_file.assert_called_with(
            'gs://test-bucket/test/path/gencode.v25.annotation.gtf.pickle',
        )
        ...
        mock_stream_gs_file.assert_called_with(
            'gs://test-bucket/test/path/gencode.v25.annotation.gtf.gz',
            raw_download=True,
        )

...
def _get_grouped_new_callset_variants(
    mt: hl.MatrixTable,
    duplicate_existing_variant_ids: set[str],
) -> itertools.groupby:
    # helper function to extract new callset variants
    # group them by existing variant id.
    mt = mt.key_rows_by(variant_id=mt.rsid)
    mt = mt.annotate_rows(
        **{
            # Downstream annotations work over nested strings.
            'info.END': mt.info.END,
            'info.END2': mt.info.END2,
            'info.CHR2': mt.info.CHR2,
        },
    )
    mt = mt.select_rows(
        end_locus=sv.end_locus(mt),
        **{'info.SEQR_INTERNAL_TRUTH_VID': mt.info.SEQR_INTERNAL_TRUTH_VID},
    )
    return itertools.groupby(
        sorted(
            mt.filter_rows(
                duplicate_existing_variant_ids.contains(
...
def deduplicate_merged_sv_concordance_calls(
    ...
    # First find the seqr variant ids that are duplicated in the new callset.
    duplicate_existing_variant_ids = hl.set(
        {
            k
    ...
    grouped_new_variants = _get_grouped_new_callset_variants(
        mt,
        duplicate_existing_variant_ids,
    )
    ...
    for existing_variant_id, new_variants in grouped_new_variants:
        ...
        for new_variant in new_variants_it1:
            distance = math.abs(
                new_variant.end_locus.position - existing_variant.end_locus.position,
            )
    ...
    return mt.annotate(
        info=mt.info.annotate(
            SEQR_INTERNAL_TRUTH_VID=hl.if_else(
                hl.set(new_variant_ids_to_exclude).contains(mt.rsid),
                None,
                mt.info.SEQR_INTERNAL_TRUTH_VID,
            ),
        ),

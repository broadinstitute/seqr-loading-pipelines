...
export APP_VERSION="$(/usr/share/google/get_metadata_value attributes/APP_VERSION)"
...
gcloud storage cp gs://seqr-pipeline-runner-builds/releases/$ENVIRONMENT/$APP_VERSION/bin/download_vep_reference_data.bash /download_vep_reference_data.bash
chmod +x /download_vep_reference_data.bash
./download_vep_reference_data.bash $REFERENCE_GENOME

gcloud storage cp gs://seqr-pipeline-runner-builds/releases/$ENVIRONMENT/$APP_VERSION/bin/vep /vep.bash
chmod +x /vep.bash

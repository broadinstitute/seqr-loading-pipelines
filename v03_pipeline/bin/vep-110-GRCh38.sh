...
gcloud storage cp --billing-oroject $PROJECT 'gs://seqr-reference-data/vep/110/Homo_sapiens.${ASSEMBLY}.dna.primary_assembly.fa.*' /vep_data/ &
...
cat > /vep_data/vep110-${ASSEMBLY}.json <<EOF
{
	"command": [
    	...
    	"--assembly", "${ASSEMBLY}",
    	"--merged",
    	"--fasta", "/opt/vep/.vep/Homo_sapiens.${ASSEMBLY}.dna.toplevel.fa.gz",
    	"--plugin", "LoF,loftee_path:/opt/vep/Plugins,gerp_bigwig:/opt/vep/.vep/gerp_conservation_scores.homo_sapiens.${ASSEMBLY}.bw,human_ancestor_fa:/opt/vep/.vep/human_ancestor.fa.gz,conservation_file:/opt/vep/.vep/loftee.sql",
    	"--dir_plugins", "/plugins",
    	"-o", "STDOUT"
	],
 	"env": {},

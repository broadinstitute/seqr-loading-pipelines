...
class TestPaths(unittest.TestCase):
    def test_family_table_path(self) -> None:
        self.assertEqual(
            family_table_path(
                Env.DEV, ReferenceGenome.GRCh37, DatasetType.SNV, 'franklin',
            ),
            'gs://seqr-scratch-temp/GRCh37/v03/SNV/families/franklin/all_samples.ht',
        )
        self.assertEqual(
            family_table_path(
                Env.PROD, ReferenceGenome.GRCh37, DatasetType.SNV, 'franklin',
            ),
    ...
    def test_variant_annotations_table_path(self) -> None:
        self.assertEqual(
            variant_annotations_table_path(
                Env.DEV, ReferenceGenome.GRCh38, DatasetType.GCNV,
            ),

...
class TestSeqrModel(unittest.TestCase):
    ...
    def test_variant_derived_fields(self):
        ...
        seqr_schema.sorted_transcript_consequences().doc_id(
            length=512
        ).variant_id().contig().pos().start().end().ref().alt().pos().xstart().xstop().xpos().transcript_consequence_terms().transcript_ids().main_transcript().gene_ids().coding_gene_ids().domains().ac().af().an().annotate_all()
        ...
        self.assertEqual(
            obj['mainTranscript']['transcript_id'],
            DERIVED_DATA[rsid]['mainTranscript']['transcript_id'],
        )
    ...
    def test_samples_gq(self):
        non_empty = {'samples_gq.75_to_80': {'NA19678'}}
        start = 0
        end = 95
        step = 5
